...
public:
	...
	void create(unsigned int width, unsigned int height, vec3 min_point, vec3 max_point,
		const char* compute_shader_file, const char* vertex_shader_file, const char* fragment_shader_file);
	void update(float delta_time, vec3 originPosition, float radius, vec3 wireFramePos);  // invoke the compute shader in the update ()
